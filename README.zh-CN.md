...
最新本地预览包：`turnmap-v0.9.0.zip`。此版本保留 ChatGPT 已验证的原生完整索引路径，并为 Gemini 加入被动原生会话索引、稳定请求/响应身份、确定性已挂载回合绑定，以及在无法无滚动、无文本猜测地解析目标时安全失败的行为。其他内置适配器继续使用有证据记录的已挂载 DOM 导航，选择器型自定义站点仍采用精确来源授权。长期备份或迁移时请优先导出 TurnMap JSON，因为它比视觉格式更完整地保留 TurnMap 专有编辑状态。
...
- `0.9.0`：当前本地预览版，加入 Gemini 被动原生索引、稳定请求/响应身份、确定性已挂载回合绑定和证据门控跳转。
