...
| Qwen | 身份优先的已挂载 DOM fallback | 精确已挂载身份跳转 | 尽力补全 | 未挂载目标安全失败，不做滚动/文本搜索。 |
| Gemini | 身份优先的已挂载 DOM fallback | 精确已挂载身份跳转 | 尽力补全 | 未挂载目标安全失败，不做滚动/文本搜索。 |
...
| GLM / Z.ai | 身份优先的已挂载 DOM fallback | 精确已挂载身份跳转 | 尽力补全 | 未挂载目标安全失败，不做滚动/文本搜索。 |
...
最新本地预览包：`turnmap-v0.8.3.zip`。此版本保留 ChatGPT 已验证的原生完整索引路径，并把其余全部内置适配器迁移到身份优先的已挂载 DOM 提取与精确已挂载目标跳转，不再使用旧式滚动/文本搜索兜底。长期备份或迁移时请优先导出 TurnMap JSON，因为它比视觉格式更完整地保留 TurnMap 专有编辑状态。
